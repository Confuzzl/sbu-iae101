...
def minuet_filename(mmid: str):
    return f"mozart/M{mmid}.wav"


def trio_filename(tmid: str):
    return f"mozart/T{tmid}.wav"


def roll_dice(num: int):
    return sum([int(random.random() * 6 + 1) for _ in range(num)])


def construct_waltz():
    minuet_measures = [minuet[roll_dice(2)] for minuet in minuet_table]
    trio_measures = [trio[roll_dice(1)] for trio in trio_table]

    buffer = b"".join([simpleaudio.WaveObject.from_wave_file(minuet_filename(
        measure)).audio_data for measure in minuet_measures + trio_measures])

    sample = simpleaudio.wave.open(minuet_filename("1"), "rb")
    play = simpleaudio.play_buffer(buffer, sample.getnchannels(),
                                   sample.getsampwidth(), sample.getframerate())
    play.wait_done()
